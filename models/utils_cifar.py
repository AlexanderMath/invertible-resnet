...
def train(args, model, optimizer, epoch, trainloader, trainset, use_cuda, train_log):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        args.writer.add_scalar("loss/loss", loss, global_step=args.i)
        ...
        if args.densityEstimation: # logging for density estimation
            ...
            args.writer.add_scalar("loss/acc", 100.*correct.type(torch.FloatTensor)/float(total), global_step=args.i)
            if batch_idx % 1 == 0:
                sys.stdout.write('\r')
